...
public interface IBudgetGovRepository extends JpaRepository <Budget_Gov, UUID> {
    ...
    @Query("SELECT b.budget FROM Budget_Gov b WHERE b.status = :status AND b.active = :active")
    double findCurrentBudgetAndActive(@Param("status") Boolean status, @Param("active") Boolean active);
